...
-- ============================================================
-- CAMBIOS TÁCTICOS BRUSCOS (descomentar para ejecutar)
-- ============================================================
-- Detecta los años donde algún equipo cambió radicalmente su
-- estilo (salto de +/- 15 puntos en velocidad o presión).
-- Estos suelen coincidir con cambios de entrenador.
-- ============================================================
/*
WITH equipos_grandes AS (
    SELECT team_api_id, team_long_name
    FROM team
    WHERE team_long_name IN (
        'FC Barcelona', 'Real Madrid CF', 'FC Bayern Munich', 'Juventus',
        'Manchester United', 'Chelsea', 'Paris Saint-Germain', 'Ajax',
        'Liverpool', 'Arsenal', 'Atlético Madrid', 'AC Milan',
...
deltas AS (
    SELECT
        ...
        velocidad,
        presion,
        velocidad - LAG(velocidad) OVER (PARTITION BY team_api_id ORDER BY anio) AS delta_velocidad,
        presion   - LAG(presion)   OVER (PARTITION BY team_api_id ORDER BY anio) AS delta_presion
    FROM stats_anuales
    WHERE rn = 1
)
SELECT
    ...
    d.delta_presion,
    CASE
        WHEN ABS(d.delta_velocidad) >= 15 AND ABS(d.delta_presion) >= 15 THEN 'Revolucion total'
        WHEN ABS(d.delta_velocidad) >= 15                                 THEN 'Cambio de ritmo'
        WHEN ABS(d.delta_presion)   >= 15                                 THEN 'Cambio defensivo'
    END AS tipo_cambio
FROM deltas d
JOIN equipos_grandes eg ON eg.team_api_id = d.team_api_id
WHERE ABS(d.delta_velocidad) >= 15
   OR ABS(d.delta_presion)   >= 15
ORDER BY GREATEST(ABS(d.delta_velocidad), ABS(d.delta_presion)) DESC;
*/